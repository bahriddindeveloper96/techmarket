    ...
    if (response.data.success) {
      ...
      const mapProduct = (product) => {
        ...
        return {
          ...
          installment: true,
          images: defaultVariant?.images || product.images?.map(img => img.image) || [product.category?.image || 'https://picsum.photos/400/400?random=1']
        }
      }
      ...
      banners.value = data.banners.map(banner => ({
        ...banner,
        image: banner.image.startsWith('http') ? banner.image : `${baseUrl}/${banner.image}`
      }))
    }
  } catch (error) {
