<template>
      ...
      <div class="grid grid-cols-1 md:grid-cols-5 gap-8 p-8">
        ...
        <div class="md:col-span-3 space-y-6">
          ...
          <div class="grid md:grid-cols-2 grid-cols-1 gap-2 relative group">
            <div v-if="selectedImage" class="aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-purple-50 to-gray-50 dark:from-purple-900/20 dark:to-gray-800 cursor-pointer relative" @click="openImageModal(selectedImage)">
            ...
            <div v-if="nextImage" class="hidden md:block aspect-square rounded-2xl overflow-hidden bg-gradient-to-br from-purple-50 to-gray-50 dark:from-purple-900/20 dark:to-gray-800 cursor-pointer relative" @click="openImageModal(nextImage)">
        ...
        <div class="md:col-span-2 p-4 space-y-4">
          ...
          <div class="bg-gradient-to-br from-purple-50 to-gray-50 dark:from-purple-900/20 dark:to-gray-800 rounded-xl p-2 space-y-2">
            <div class="flex items-baseline gap-2">
              ...
              <span v-if="product.oldPrice" class="text-base text-gray-400 dark:text-gray-500 line-through">
        ...
        <div class="grid grid-cols-1 md:grid-cols-5 gap-8">
          <div class="md:col-span-3 space-y-4">
            ...
            <div class="grid grid-cols-5 gap-2">
              <div v-for="n in 5" :key="n" class="aspect-square bg-gray-200 dark:bg-gray-700 rounded-xl"></div>
            </div>
          </div>
          <div class="md:col-span-2 space-y-4">
            ...
            <div class="space-y-2">
              <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded w-1/4"></div>
              <div class="flex gap-2">
                <div v-for="n in 4" :key="n" class="w-8 h-8 bg-gray-200 dark:bg-gray-700 rounded-full"></div>
              </div>
            </div>
            <div class="space-y-2">
              <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded w-1/4"></div>
              <div class="flex gap-2">
                <div v-for="n in 4" :key="n" class="w-16 h-10 bg-gray-200 dark:bg-gray-700 rounded-lg"></div>
              </div>
            </div>
            <div class="h-12 bg-gray-200 dark:bg-gray-700 rounded"></div>
          </div>
  ...
  <!-- Image Modal -->
  <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90" @click="closeImageModal">
    <button @click="showPrevImage" class="absolute left-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
      <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
      </svg>
    </button>
    
    <button @click="showNextImage" class="absolute right-4 top-1/2 -translate-y-1/2 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
      <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
      </svg>
    </button>

    <button @click="closeImageModal" class="absolute top-4 right-4 p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
      <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
      </svg>
    </button>

    <img :src="modalImage" :alt="product?.name" class="max-w-full max-h-[90vh] object-contain" @click.stop />
  </div>
   <!-- Similar Products -->
   <div class="mt-2">
...
const fetchProduct = async () => {
  try {
    const response = await axios.get(`${baseUrl}/api/products/${route.params.id}`, {
      headers: {
    ...
    if (response.data.message === "Product retrieved successfully") {
      ...
      product.value = {
        ...
        reviews: 0,
        orders: 0,
        colors: [...new Set(productData.variants?.map(v => v.attribute_values.Color))],
        sizes: [...new Set(productData.variants?.map(v => v.attribute_values.Storage))],
        images: productData.variants?.[0]?.images || productData.images?.map(img => img.image) || []
      }
...
const updateSelectedVariant = () => {
  if (!selectedColor.value || !selectedSize.value || !product.value?.variants) return
  
  const newVariant = product.value.variants.find(v => 
